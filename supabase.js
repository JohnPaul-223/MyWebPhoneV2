...
/* ═══════════════════════════════════════════════════════════
   INCOMING CALLS - For real-time call notifications
═══════════════════════════════════════════════════════════ */

async function createIncomingCall(toPhone, fromPhone, callType) {
  console.log("📞 [Supabase] Creating incoming call record:", { to_phone: toPhone, from_phone: fromPhone, call_type: callType });
  const { data, error } = await supabaseClient
    .from("incoming_calls")
    .insert([{ to_phone: toPhone, from_phone: fromPhone, call_type: callType }])
    .select();
  
  if (error) {
    console.error("❌ [Supabase] Failed to create incoming call:", error);
    throw new Error(error.message);
  }
  console.log("✅ [Supabase] Incoming call created successfully:", data[0]);
  return data[0];
}

async function updateIncomingCall(id, status) {
  const updates = { status };
  if (status === "accepted") updates.answered_at = new Date();
  if (status === "rejected" || status === "missed") updates.ended_at = new Date();
...
function subscribeToIncomingNotifications(toPhone, callback) {
  console.log("📡 [Supabase] Creating notification listener for phone:", toPhone);
  console.log("📡 [Supabase] Watching table: incoming_calls where to_phone =", toPhone);
  
  const subscription = supabaseClient
    .channel(`incoming_calls:${toPhone}`)
    .on(
      "postgres_changes",
      {
        event: "INSERT",
        ...
        filter: `to_phone=eq.${toPhone}`
      },
      (payload) => {
        console.log("✨ [Supabase] Incoming call notification received:", payload.new);
    ...
    .subscribe((status, err) => {
      if (status === "SUBSCRIBED") {
        ...
        console.log("⚠️ [Supabase] Subscription closed");
