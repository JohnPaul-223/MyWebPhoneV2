...
function enterApp(account) {
  ...
  setHint("Enter the other user's phone number or username, then click Call.");
  setStatus("Ready");
...
const waitBtn         = null; // REMOVED - using call type modal instead
...
function endCall(localAction = false) {
  ...
  resetState();
  setHint("Enter a number and click Call to connect.");
}
...
function readDialNumberOrExplain() {
  ...
  if (!value) {
    ...
    setHint("Type the other user's number, then click Call.");
    return null;
  }
  return value;
}
...
async function startCaller(withVideo = true) {
  ...
  updateControls();
  setStatus("Dialing…");
  setHint("Recipient must enter your number and click Call to accept.");
  ...
  peer.on("open", () => {
    ...
    call.on("error", (err) => {
      ...
      setHint("Recipient: Enter the caller's number, then Call to accept.");
      closeActiveCall(); resetState();
      if (peer) { peer.destroy(); peer = null; }
    });
  });
  ...
  peer.on("error", (err) => {
    ...
    if (!errorShown && err.type !== "unavailable-id") {
      ...
      setHint("Make sure receiver has entered this number. Refresh and try again.");
      endCall(false);
    }
  });
}
...
async function startReceiver() {
  ...
  setHint("Waiting for incoming call from another device…");
  ...
  peer.on("error", (err) => {
    ...
    if (err.type === "unavailable-id") {
      ...
      setHint("Someone is already waiting on this number. Try a different one.");
    }
    endCall(false);
  });
}
...
// Removed waitBtn.addEventListener for call type modal instead
