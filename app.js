...
function enterApp(account) {
  ...
  setHint("Device A: enter number → Call → pick audio/video. Device B: will get call notification.");
  setStatus("Ready");
...
function endCall(localAction = false) {
  ...
  resetState();
  setHint("Device A: enter number → Call. Device B: same number → Wait.");
}
...
function readDialNumberOrExplain() {
  ...
  if (!value) {
    ...
    setHint("Enter the other device's number, then click Call or Wait.");
    return null;
  }
  return value;
}
  ...
  peer.on("open", () => {
    ...
    if (currentAccount) {
      ...
      createIncomingCall(value, currentAccount.phone, callType)
        .catch(err => console.error("Failed to create incoming call notification:", err));
    }
...
async function startReceiver() {
  ...
  setHint("Phone number loaded — you'll get a notification when someone calls.");

  const receiverId = createPeerId(value);
  peer = new Peer(receiverId);
  console.log("Receiver peer ID:", receiverId);
  
  // Subscribe to incoming call notifications from Supabase
