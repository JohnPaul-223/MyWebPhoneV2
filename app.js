...
function doLogin() {
  ...
  if (!account || account.passwordHash !== hashSimple(password)) {
    ...
    console.error("Login failed - account not found or password mismatch");
    return;
  }
...
function enterApp(account) {
  console.log("enterApp called for:", account.username);
  ...
  console.log("Auth hidden:", authOverlay.classList.contains("auth-overlay--hidden"));
  console.log("App shell hidden:", appShell.hidden);

  // Kick off map + devices
  ensureMap();
  initDeviceListsIfPossible();
  setHint("Enter a number and tap Call. Incoming calls are listened to in the background.");
  setStatus("Starting background listener...");
  ...
  // Always keep this signed-in user listening in the background.
  maybeResumeReceiverListening();
...
logoutBtn.addEventListener("click", () => {
  ...
  if (inCall || peer || activeCall) endCall(false, false);
...
const closeCallTypeModal = document.getElementById("closeCallTypeModal");
const audioCallBtn       = document.getElementById("audioCallBtn");
const videoCallBtn       = document.getElementById("videoCallBtn");
const callTypeNumber     = document.getElementById("callTypeNumber");
...
let currentIncomingCall  = null; // store incoming call data
let notificationSubscription = null; // store the realtime subscription
let receiverBooting = false;
...
function normalizeDialNumber(value) {
  return String(value || "").replace(/\D/g, "").slice(0, MAX_DIAL_LEN);
}

function getCurrentAccountListenNumber() {
  return normalizeDialNumber(currentAccount?.phone || "");
}
...
function stopNotificationSubscription() {
  ...
  try {
    ...
    console.warn("Failed to unsubscribe notification listener:", err);
  }
...
function endCall(localAction = false, resumeBackgroundListening = true) {
  if (localAction) setStatus("Call ended");
  // Log to history
  if (callLive && dialBuffer && myRole) {
  ...
  resetState();
  setHint("Call ended.");
  if (resumeBackgroundListening) maybeResumeReceiverListening();
}

function maybeResumeReceiverListening() {
  const listenNumber = getCurrentAccountListenNumber();
  if (!listenNumber || receiverBooting) return;
  if (peer || activeCall || inCall) return;
  startReceiver(listenNumber, { silent: true }).catch((err) => {
    console.error("Background listener restart failed:", err);
  });
}
  ...
  peer.on("call", async (incomingCall) => {
    ...
    if (!localStream) {
      try {
        ...
        console.log("Rejecting - no local stream");
        setStatus("Local media not ready.");
        incomingCall.close();
        maybeResumeReceiverListening();
        return;
      }
    ...
    if (activeCall) {
      console.log("Rejecting - already in call");
      incomingCall.close();
      return;
    }
    ...
    myRole = "receiver";
    dialBuffer = incomingCall.peer.replace("pc-phone-", "").split("-caller")[0];
    syncDialDisplay();
    inCall = true;
    mountVideosForReceiver();
    updateControls();
    setStatus("Connecting...");
    notifyBackgroundUser("Incoming Call", "Someone is calling!");
    incomingCall.answer(localStream);
    setHint("Answering call...");
...
function readDialNumberOrExplain() {
  const value = normalizeDialNumber(dialBuffer.trim());
  if (!value) {
    setStatus("Enter a number on the keypad first.");
    setHint("Enter the other device's number, then tap Call.");
    return null;
  }
  return value;
}
...
async function startCaller(withVideo = true) {
  const value = readDialNumberOrExplain();
  if (!value) return;
  if (peer || activeCall) endCall(false, false);
  ...
  // If audio-only call, disable video tracks
  ...
  updateControls();
  setStatus("Calling...");
  setHint("Calling - they'll see a notification to accept.");
  ...
  peer.on("open", () => {
    console.log("Caller peer open:", ownId);

    // Create incoming call notification in Supabase
    if (currentAccount) {
      const callType = withVideo ? "video" : "audio";
      console.log("Attempting to send notification to:", value, "from:", currentAccount.phone, "type:", callType);
      createIncomingCall(value, currentAccount.phone, callType)
        .then(result => {
          console.log("Notification sent successfully:", result);
        })
        .catch(err => {
          console.error("Failed to create incoming call notification:", err);
          setStatus("Failed to notify receiver");
          setHint("Supabase error: " + err.message);
        });
    } else {
      console.warn("No currentAccount, cannot send notification");
    }
    ...
    call.on("error", (err) => {
      ...
      setStatus("Call failed - no one picked up.");
      setHint("Make sure the other device is listening on this number.");
      endCall(false);
    });
  });
  ...
  peer.on("error", (err) => {
    ...
    if (!errorShown && err.type !== "unavailable-id") {
      errorShown = true;
      setStatus("Connection error.");
      setHint("Connection failed. Check your number and try again.");
      endCall(false);
    }
  });
}

async function startReceiver(listenNumber = null, options = {}) {
  const { silent = false } = options;
  const sourceNumber = listenNumber ?? (silent ? getCurrentAccountListenNumber() : readDialNumberOrExplain());
  const value = normalizeDialNumber(sourceNumber);
  if (!value) return false;
  if (receiverBooting) return false;

  receiverBooting = true;
  try {
    if (peer || activeCall) endCall(false, false);
    ...
    inCall = false;
    updateControls();
    setStatus("Listening for calls...");
    setHint("Background listener active on your number.");

    const receiverId = createPeerId(value);
    peer = new Peer(receiverId);
    console.log("Receiver peer ID:", receiverId);

    stopNotificationSubscription();
    console.log("Setting up notification listener for phone:", value);
    try {
      notificationSubscription = subscribeToIncomingNotifications(value, (callData) => {
        console.log("Got incoming call notification:", callData);
        showIncomingCallNotification(callData);
      });
      console.log("Notification listener activated for:", value);
    } catch (err) {
      console.error("Failed to set up notification listener:", err);
      setStatus("Notification setup failed");
      setHint("Try refreshing the page. Error: " + err.message);
    }
    ...
    peer.on("error", (err) => {
      ...
      if (err.type === "unavailable-id") {
        setStatus("Number in use.");
        setHint("This number is already in use. Try a different one.");
        endCall(false, false);
        return;
      }
      endCall(false);
...
function showIncomingCallNotification(callData) {
  ...
  incomingCallNumber.textContent = callData.from_phone;
  incomingCallType.textContent = callData.call_type === 'video' ? 'Video call' : 'Audio call';
...
callBtn.addEventListener("click", () => {
  ...
  callTypeNumber.textContent = value;
  callTypeModal.hidden = false;
});
...
/* ═══════════════════════════════════════════════════════════
   CALL TYPE SELECTION MODAL - Event Listeners
═══════════════════════════════════════════════════════════ */
closeCallTypeModal.addEventListener("click", () => { callTypeModal.hidden = true; pendingCallType = null; });
callTypeModal.addEventListener("click", e => { if (e.target === callTypeModal) { callTypeModal.hidden = true; pendingCallType = null; } });

audioCallBtn.addEventListener("click", async () => {
  callTypeModal.hidden = true;
  pendingCallType = "audio";
  await startCaller(false); // false = audio only
...
videoCallBtn.addEventListener("click", async () => {
  ...
  await startCaller(true); // true = with video
  pendingCallType = null;
});

acceptCallBtn.addEventListener("click", async () => {
  console.log("User clicked Accept for incoming call");
  incomingCallModal.hidden = true;
  if (currentIncomingCall) {
    try {
      await updateIncomingCall(currentIncomingCall.id, "accepted");
    } catch (err) {
      console.error("Failed to update call status:", err);
    }
  }
  // The WebRTC connection should already be incoming via bindIncomingCalls()
});

rejectCallBtn.addEventListener("click", async () => {
  console.log("User clicked Reject for incoming call");
  incomingCallModal.hidden = true;
  if (currentIncomingCall) {
    try {
      await updateIncomingCall(currentIncomingCall.id, "rejected");
    } catch (err) {
      console.error("Failed to update call status:", err);
    }
    currentIncomingCall = null;
